...
import { CreditCard, AlertCircle, CheckCircle, Clock, Search } from 'lucide-react';
...
export default function PaymentsPage() {
  ...
  return (
    <div className="p-4 lg:p-8 max-w-5xl mx-auto animate-fade-in">
      <div className="mb-6">
        <h1 className="font-display text-3xl font-semibold">Payments & Receivables</h1>
        <p className="text-sm text-muted-foreground mt-1">Track what's owed and what's collected</p>
      </div>

      {/* Summary */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <div className="stat-card">
          <div className="flex items-center gap-2 text-muted-foreground mb-2"><CreditCard className="w-4 h-4" /><span className="text-xs uppercase tracking-wider">Total Outstanding</span></div>
          <p className="font-display text-2xl font-semibold text-destructive">{formatCurrency(totalOutstanding)}</p>
        </div>
        <div className="stat-card">
          <div className="flex items-center gap-2 text-muted-foreground mb-2"><AlertCircle className="w-4 h-4" /><span className="text-xs uppercase tracking-wider">Overdue</span></div>
          <p className="font-display text-2xl font-semibold text-destructive">{formatCurrency(overdue.reduce((s, i) => s + i.amount, 0))}</p>
        </div>
        <div className="stat-card">
          <div className="flex items-center gap-2 text-muted-foreground mb-2"><Clock className="w-4 h-4" /><span className="text-xs uppercase tracking-wider">Upcoming</span></div>
          <p className="font-display text-2xl font-semibold">{formatCurrency(pending.reduce((s, i) => s + i.amount, 0))}</p>
        </div>
        <div className="stat-card">
          <div className="flex items-center gap-2 text-muted-foreground mb-2"><CheckCircle className="w-4 h-4" /><span className="text-xs uppercase tracking-wider">Total Collected</span></div>
      ...
      <div className="flex gap-1 border-b border-border mb-6">
        {(['overview', 'history', 'plans'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} className={`px-4 py-2.5 text-sm font-medium capitalize border-b-2 -mb-px transition-colors ${tab === t ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>
            {t === 'plans' ? 'Installment Plans' : t === 'history' ? 'Payment History' : 'By Client'}
          </button>
        ))}
      </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Last payment: {client.lastPurchase ? formatDate(client.lastPurchase) : '—'}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-lg font-display font-semibold text-destructive">{formatCurrency(client.outstanding)}</p>
                  <button className="text-xs text-primary hover:underline mt-1">Mark payment</button>
                </div>
              </div>
            </div>
          ))}
